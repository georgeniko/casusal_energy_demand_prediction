#This is the implementation of the multivariate LSTM model with all the 
#available variables.
...
# The multivariate LSTM is not much different from the univariate one in terms of implementation. 
# The main difference is the size of the input, which must be modified so as the function can 
# recognize the different features. 

# The process of algorithmic implementation, prediction and validation remains constant with the 
# univariate. Specifically, hourly forecasts are implemented for a day ahead. The training set 
# is then moved by a day ahead and the process is repeated for all available data.
...
x, y = f.df_to_X_y2(data, WINDOW_SIZE)   
...
for train_idx, val_idx in tss.split(x):
    ...
    if number_of_fold<=splits:
        ...
        if len(x_test)!=0:
            ...
            combined = np.column_stack((y_pred1, y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1,y_pred1))
            predictions = sc.inverse_transform(combined)

            combined2 = np.column_stack((y_test, y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test,y_test))
...
hourle_evaluations= pd.DataFrame(hourly_errors,columns=cols)
rmse=[]
mae=[]
mape=[]
...
# scores.to_excel('LSTMf_multistep_SCORES_GER.xlsx', sheet_name='LSTMf_SCORES_GER')
# hourly_scores.to_excel('LSTMf_HOURLY_SCORES_GER.xlsx', sheet_name='LSTMf_HOURLY_SCORES_GER')
# hourle_evaluations.to_excel('LSTMf_HOURLY_ERRORS_GER.xlsx', sheet_name='LSTMf_HOURLY_ERRORS_GER')
# full_real_df.to_excel('LSTMf_HOURLY_REAL_GER.xlsx', sheet_name='LSTMf_HOURLY_REAL_GER')
